...
const getAnchorAndDir = (origin: RaysOrigin, w: number, h: number) => {
  ...
  switch (origin) {
    ...
    case "top-right-down": {
      const anchor: [number, number] = [(1 + outside * 0.3) * w, -outside * h];
      const target: [number, number] = [0.35 * w, 0.55 * h];
      const dir = normalize(target[0] - anchor[0], target[1] - anchor[1]);
      return { anchor, dir };
    }
    case "left":
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  fragColor = (rays1 * 0.5 + rays2 * 0.4) * intensity;
  ...
  vec3 chromaBias = vec3(0.35, 0.55, 0.85);
  fragColor.rgb *= chromaBias + brightness * vec3(0.9, 0.95, 1.05);
...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const loop = (t: number) => {
        ...
        uniformsRef.current.iTime.value = t * 0.0001;
