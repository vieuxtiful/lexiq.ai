...
function CareersParticles() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < positionsArr.length; i += 3) {
      ...
      // Soft banded flow roughly echoing the globe latitude lines
...
function CareersGlobe() {
  ...
  useFrame((_, delta) => {
    const spinSpeed = 0.45; // radians per second
    ...
    if (nextRev > prevRev) {
      // Advance to next region glyph once per full revolution
    ...
    const normalizedAngle = ((spinAngleRef.current % fullRotation) + fullRotation) % fullRotation; // 0 -> 2π
    const signedFrontAngle = ((normalizedAngle + Math.PI) % fullRotation) - Math.PI; // -π -> π
  ...
  return (
    <group ref={globeGroupRef} position={[0.73, 0.16, 0]}>
      {/* Core globe */}
      ...
      {/* Subtle latitude/longitude lines via slightly larger wireframe sphere */}
      <mesh>
        <sphereGeometry args={[0.899, 32, 32]} />
        <meshBasicMaterial color="#0ea5e9" wireframe opacity={0.28} transparent />
      </mesh>
...
export function CareersPageBackgroundV1() {
  return (
    <div
